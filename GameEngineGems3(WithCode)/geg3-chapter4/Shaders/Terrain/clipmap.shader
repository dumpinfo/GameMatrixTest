...
uniform int texturelookupmode=0;
...
vec4 TerrainTextureLookup(in sampler2D texmap, in vec3 coord, in vec3 axis)
{
	...
	case 1:
		return texture(texmap,coord.xy) * axis.z + texture(texmap,coord.zy) * axis.x;
	case 2:
		return texture(texmap,coord.xy) * axis.z + texture(texmap,coord.zy) * axis.x + texture(texmap,coord.xz) * axis.y;
	}
}

vec4 TerrainNormalLookup(in sampler2D texmap, in vec3 coord, in vec3 axis)
{
	vec4 color;
	switch (texturemappingmode)
	{
	case 0:
		...
		break;
	case 1:
		color = texture(texmap,coord.xy).xyzw * axis.z + texture(texmap,coord.zy).yxzw * axis.x;
		break;
	case 2:
		color = texture(texmap,coord.xy).xyzw * axis.z + texture(texmap,coord.zy).yxzw * axis.x + texture(texmap,coord.xz) * axis.y;		
		break;
	}
	return color;
}
...
void main(void)
{
	...
	vec3 axis = abs(normal.xzy);
	if (texturemappingmode != 2)
	{
		axis.y=0.0;
		axis=normalize(axis);
	}
	axis=pow(axis,vec3(50));//increase this value for harder mapping plane transitions
	float nsum = axis.x + axis.y + axis.z;
	axis /= nsum;
	...
	// Normal
	//outcolor1 = texture(texture6,texcoords.xz);
	outcolor1 = TerrainNormalLookup(texture6,texcoords,axis);
#ifdef TERRAIN_LOW_FREQUENCY_BLEND
		outcolor1 = outcolor1*lowfrequencymix + TerrainNormalLookup(texture6,texcoords/TERRAIN_LOW_FREQUENCY_BLEND,axis)*(1.0-lowfrequencymix);
#endif
	...
	{
		displacement = TerrainTextureLookup(texture7,texcoords,axis).r;
#ifdef TERRAIN_LOW_FREQUENCY_BLEND
		displacement = displacement*lowfrequencymix + TerrainTextureLookup(texture7,texcoords/TERRAIN_LOW_FREQUENCY_BLEND,axis).r*(1.0-lowfrequencymix);
#endif
	...
	// Diffuse
	//outcolor = texture(texture5,texcoords.xz);
	outcolor = TerrainTextureLookup(texture5,texcoords,axis);
#ifdef TERRAIN_LOW_FREQUENCY_BLEND
	outcolor = outcolor*lowfrequencymix + TerrainTextureLookup(texture5,texcoords/TERRAIN_LOW_FREQUENCY_BLEND,axis)*(1.0-lowfrequencymix);
#endif
	
	fragData0 = vec4(outcolor.rgb,alpha);
	fragData1 = vec4(outcolor1.rgb,alpha);
	
	//fragData0.a = displacement;//
