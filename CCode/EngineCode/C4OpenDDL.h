...
namespace C4
{
	...
	//
	//# \def	namespace C4 { namespace Data {...} } 
	//
	//# \also	$@Structure@$ 
	...
	//
	//# \proto	DataResult ReadIdentifier(const char *text, int32 *textLength, char *restrict identifier = nullptr);
	//
	//# \param	text			A pointer to a text string.
	//# \param	textLength		A pointer to the location that receives the length of the identifier.
	//# \param	identifier		A pointer to a buffer that will receive the identifier string. If this is $nullptr$, then the identifier string itself is not returned.
	//
	//# \desc
	...
	namespace Data
	{
		...
		C4API int32 GetWhitespaceLength(const char *text);
		C4API DataResult ReadDataType(const char *text, int32 *textLength, DataType *value);
		C4API DataResult ReadIdentifier(const char *text, int32 *textLength, char *restrict identifier = nullptr);
		C4API DataResult ReadStringLiteral(const char *text, int32 *textLength, int32 *stringLength, char *restrict string = nullptr);
		C4API DataResult ReadBoolLiteral(const char *text, int32 *textLength, bool *value);
		C4API DataResult ReadUnsignedLiteral(const char *text, int32 *textLength, unsigned_int64 *value);
		C4API DataResult ReadFloatMagnitude(const char *text, int32 *textLength, Half *value);
		C4API DataResult ReadFloatMagnitude(const char *text, int32 *textLength, float *value);
		C4API DataResult ReadFloatMagnitude(const char *text, int32 *textLength, double *value);
	}
	...
	class StructureRef
	{
		...
		public:
			...
			C4API StructureRef(bool global = true);
			C4API ~StructureRef();
			...
			C4API void Reset(bool global = true);
	};
	...
	class Structure : public Tree<Structure>, public MapElement<Structure>
	{
		...
		protected:
			...
			C4API Structure(StructureType type);
		...
		public:
			...
			C4API Structure *GetFirstSubstructure(StructureType type) const;
			C4API Structure *GetLastSubstructure(StructureType type) const;

			C4API Structure *FindStructure(const StructureRef& reference, int32 index = 0) const;

			C4API virtual bool ValidateProperty(const DataDescription *dataDescription, const String<>& identifier, DataType *type, void **value);
			C4API virtual bool ValidateSubstructure(const DataDescription *dataDescription, const Structure *structure) const;

			C4API virtual DataResult ProcessData(DataDescription *dataDescription);
	};
	...
	class DataDescription
	{
		...
		protected:
			...
			C4API DataDescription();

			C4API virtual DataResult ProcessData(void);

		public:

			C4API virtual ~DataDescription();
			...
			C4API Structure *FindStructure(const StructureRef& reference) const;

			C4API virtual Structure *CreateStructure(const String<>& identifier) const;
			C4API virtual bool ValidateTopLevelStructure(const Structure *structure) const;

			C4API DataResult ProcessText(const char *text);
	};
}
